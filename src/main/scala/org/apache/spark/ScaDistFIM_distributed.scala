...
object ScaDistFIM_distributed {
  def main(args: Array[String]): Unit = {
    ...
    // 指定要检查的目录路径
    val outputPath = new Path("output1/data")
    // 检查目录是否存在
    ...
    val value1: RspRDD[Row] = dataRead.rdd.toRSP(10000).getSubPartitions(500).toRSP(2000)
    spark.createDataFrame(value1, spark.rspRead.parquet("/user/caimeng/Items_10E_10000_100000.parquet").schema).write.parquet("output1/data")
    val numberOfData: Double = value1.getSubPartitions(1).count().toDouble

    val transactions: RspRDD[Array[Array[Int]]]  = spark.rspRead.parquet("output1/data")
